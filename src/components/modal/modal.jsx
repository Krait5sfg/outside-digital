...
import {calculateTaxDeductionForYear, calculateYearCount, LIMIT_NUMBER_LENGTH} from '../../utils/utils';
import NumberFormat from 'react-number-format';

const Modal = ({isModalActive, onModalButtonCloseClick, onModalButtonSubmitClick}) => {
  ...
  const handleCalculateButtonClick = (evt) => {
    ...
    if (salaryInputValue.toString().length < LIMIT_NUMBER_LENGTH) {
      return false;
    }
  ...
  const handleSubmitButtonClick = (evt) => {
    ...
    if (isCheckboxActive) {
      ...
      xhr.send(formData);
      setSalaryInputValue(``); // Очищает инпут с зарплатой
      setCheckboxActive(false); // Закрывает чекбокс
      onModalButtonSubmitClick(); // закрывает модальное окно при отправке формы
    }
  };
  ...
  return (
    <div className={isModalActive ? `modal` : `modal modal--not-active`}>
      <div className="modal__wrapper">
        <button className="modal__close" aria-label="Закрыть форму" onClick={onModalButtonCloseClick}>
        ...
        <form className="modal__form" action="/" method="post" name="form" ref={formRef}>
          ...
          <NumberFormat className={classNameModalInputSalaryElement} placeholder="Введите данные" id="salary" name="salary" thousandSeparator=" " suffix=" ₽" onValueChange={handleSalaryInput} allowNegative={false} title="Сумма должна быть минимум из пяти цифр" value={salaryInputValue} isNumericString={true} />
          ...
          <button className={isCheckboxActive ? `modal__submit modal__submit--sm-margin` : `modal__submit`}
            type="submit"
            onClick={handleSubmitButtonClick}>Добавить</button>
        </form >
      </div>
    </div>
  )
